...
def split_by_seconds(filename, split_length, vcodec="copy", acodec="copy",
    ...
    split_cmd = ['ffmpeg']
    split_args = ["-i", filename, "-vcodec", vcodec, "-acodec", acodec] + shlex.split(extra)
    try:
    ...
    for n in range(0, split_count):
        ...
        split_add_args += ["-ss", str(split_start), "-t", str(split_length),
                       filebase + "-" + str(n + 1) + "-of-" + \
                       str(split_count) + "." + fileext]
        print("About to run: " + " ".join(split_cmd + split_add_args + split_args))
        subprocess.check_output(split_cmd + split_add_args + split_args)
